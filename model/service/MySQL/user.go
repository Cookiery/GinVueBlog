...
type User struct {
	gorm.Model
	UserName string `gorm:"type:varchar(20);not null" json:"userName" validate:"required,min=4,max=12"` // 用户名
	Password string `gorm:"type:varchar(20);not null" json:"password" validate:"required,min=6,max=20"` // 密码
	Role     int    `gorm:"type:int;DEFAULT:2" json:"role" validate:"required,gte=2"`                   // 角色
}
...
func GetUsers(pageSize int, pageNum int) ([]User, int64) {
	...
	// 分页
	err = db.Table(usersTable).Limit(pageSize).Offset(offset).Find(&users).Count(&total).Error
